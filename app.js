...
/** Formate un nombre en notation compacte  ex: 1200 → 1.2k */
function fmt(n) {
...
function toggleBookmark() {
    ...
    if (idx === -1) {
        ...
        showToast(`${u.login} ajouté aux favoris`, '★');
    } else {
        // Suppression
        state.bookmarks.splice(idx, 1);
        showToast(`${u.login} retiré des favoris`, '🗑️');
    }
...
// Exposé sur window car appelé via onclick dans le HTML généré dynamiquement
window.removeBookmark = function (login, e) {
    e.stopPropagation();
    state.bookmarks = state.bookmarks.filter((b) => b.login !== login);
    saveBookmarks();
    renderBookmarks();
    showToast(`${login} retiré des favoris`, '🗑️');
...
function renderBookmarks() {
    ...
    if (state.bookmarks.length === 0) {
        list.innerHTML = `
            <div class="bm-empty">
                ...
                Aucun favori.<br/>Recherchez un profil et sauvegardez-le.
            </div>`;
        return;
    }
    ...
    list.innerHTML = state.bookmarks.map((b) => `
        <div class="bm-card" onclick="loadBookmark('${escHtml(b.login)}')">
            <img class="bm-avatar" src="${escHtml(b.avatar_url)}" alt="${escHtml(b.login)}" />
            <div class="bm-info">
                <div class="bm-login">${escHtml(b.login)}</div>
                <div class="bm-name">${escHtml(b.name || '—')}</div>
            </div>
            <button class="bm-remove" title="Retirer"
                    onclick="removeBookmark('${escHtml(b.login)}', event)">
                &times;
            </button>
        </div>
    `).join('');
...
function bindEvents() {
    ...
    // Bouton favori sur le profil affiché
    document.getElementById('r-bookmark-btn')
